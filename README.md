...
- Deprecated-format detection via the plugin command `_o_AP_Is_Picture_Deprecated`.
...
- **`_o_AP_Is_Picture_Deprecated`** -- a plugin command used to detect a deprecated picture format; called once per candidate format in a loop, passed a picture variable by reference (`->$pictureInSignleFormat`).
