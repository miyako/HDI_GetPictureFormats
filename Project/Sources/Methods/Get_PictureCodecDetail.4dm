...
For ($i; 1; Size of array:C274($PictureCodecIDList))
	...
	If (Find in array:C230($SupportedOSCodecIDList; $PictureCodecIDList{$i})#-1)
		...
		C_LONGINT:C283($isPictureDeprecated)
		...
		If ($isPictureDeprecated=1)
			txtCodecDetail:=txtCodecDetail+"   - "+$PictureCodecIDList{$i}+": is deprecated  \n"
			// else the format is fully supported
		Else 
			txtCodecDetail:=txtCodecDetail+"   - "+$PictureCodecIDList{$i}+": is supported \n"
		End if 
		//if the codec is NOT supported
	Else 
		txtCodecDetail:=txtCodecDetail+"   - "+$PictureCodecIDList{$i}+": is NOT supported\n"
	End if 
